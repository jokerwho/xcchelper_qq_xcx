...
<view class="district {{district_open ? 'show' : 'disappear'}} ">
  <view class="half half-left">
    <view class="{{district_left_select == item.id ? 'current_left_select' : ''}}" qq:for="{{dropDownMenuDistrictData}}" bindtap="selectDistrictLeft" data-model='{{item}}' qq:key="unique">
      {{item.title}}
    </view>
  </view>
  <view class="half half-right">
    <view class="{{district_right_select == item.id ? 'current_right_select' : ''}}" qq:for="{{dropDownMenuDistrictDataRight}}" bindtap="selectDistrictRight" data-model='{{item}}' qq:key="unique">
      {{item.title}}
    </view>
  </view>
</view>
...
<view class="container container_hd {{source_open ? 'show' : 'disappear'}} ">
  <view class='z-height'>
    <view>
      <block qq:for="{{dropDownMenuSourceData}}" qq:key="unique">
...
<view class="container container_hd {{style_open ? 'show' : 'disappear'}} ">
  <view class='z-height'>
    <view>
      <block qq:for="{{dropDownMenuStyleData}}" qq:key="unique">
...
<view class="container container_hd {{filter_open ? 'show' : 'disappear'}} ">
  <view class='z-height'>
    <view>
      <block qq:for="{{dropDownMenuFilterData}}" qq:key="unique">
